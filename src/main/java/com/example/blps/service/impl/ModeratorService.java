...
public class ModeratorService implements IModeratorService {
    ...
    @Override
    public Question acceptQuestion(UUID id) throws NotFoundException, GeneralValidationException {
        if(!questionRepository.existsById(id)){
            throw  new NotFoundException("Question with UUID " + id + " not found.");
        }
        ...
        if(!question.isNeedsModeration()){
            throw new GeneralValidationException("Question with UUID "+id +" is already accepted. ");
        }
